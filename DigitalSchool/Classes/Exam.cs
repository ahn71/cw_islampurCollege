       ...
       public static  DataTable dt;
        ...
        public static DataTable loadMarks(string ClassName, string ExamID, string BatchID, string ShiftID, string RollNo)
        {
            try
            {
                ...
                string query = @"SELECT  m.StudentId,m.SubId,m.CourseId,SUM(m.Marks) AS Marks,SUM(CASE WHEN m.IsPassed = 1 THEN 0 ELSE 1 END) AS IsFailed,cs.Marks AS FullMarks FROM (SELECT DISTINCT StudentId, SubId, CourseId, Marks, IsPassed, BatchId, ExamID, ShiftID, RollNo FROM Class_TwelveMarksSheet) m INNER JOIN BatchInfo b ON m.BatchId = b.BatchId INNER JOIN ClassSubject cs ON b.ClassID = cs.ClassID AND m.SubId = cs.SubId AND m.CourseId = cs.CourseId WHERE m.BatchId = '"+ BatchID + "'AND m.ExamID = '"+ ExamID + "' " + ShiftID + " " + RollNo + " GROUP BY  m.StudentId, m.SubId, m.CourseId, cs.Marks ORDER BY m.StudentId;";
